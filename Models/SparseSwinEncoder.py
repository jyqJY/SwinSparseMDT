...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        qk_transposed = torch.matmul(q, k.transpose(2, 3)) / np.sqrt(
            self.head_dims)
        attention_weights = torch.softmax(qk_transposed, dim=-1)

        attention_weights = self.att_drop(attention_weights)

        weighted_avg = torch.matmul(attention_weights, v)

        weighted_avg = weighted_avg.transpose(1, 2).flatten(start_dim=2)
        out = self.lin(weighted_avg)

        out = self.lin_drop(out)
...
class SparseToken(nn.Module):
    ...
    def forward(self, x):
        ...
        sparse_token = self.convert(x)

        sparse_token = sparse_token.flatten(start_dim=2)
...
class SparseSwinEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        step4, attn_weights = self.step4(block3)
        out = self.norm(step4)
        out = self.token2spatial(out)
