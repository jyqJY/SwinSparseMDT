...
class SparseToken(nn.Module):
    ...
    def forward(self, x):
        ...
        sparse_token_dim_converter = self.convert(x)

        sparse_token = sparse_token_dim_converter.flatten(start_dim=2)
...
class SparseSwin(nn.Module):
    ...
    def __init__(self, swin_type, num_classes, c_dim_3rd, hw_size_3rd,
                    ...
                    lin_drop_prob=0.0, freeze_12=False, device='cuda', tree_depth=3):
        ...
        self.decision_tree = SoftDecisionTree(input_dim=ltoken_dims, depth=tree_depth, num_classes=num_classes)


    def forward(self, x):
        swin_out = self.swin_model(x)

        step4, attn_weights, sparse_token_dim_converter = self.step4(swin_out)
        ...
        tree_out = self.decision_tree(out)
        return tree_out, attn_weights, sparse_token_dim_converter
...
class SoftDecisionTree(nn.Module):
    def __init__(self, input_dim, depth, num_classes):
        ...
        self.inner_nodes = 2 ** depth - 1
        self.leaf_nodes = 2 ** depth


        self.node_weights = nn.Parameter(torch.randn(self.inner_nodes, input_dim) * 0.1)
        self.node_bias = nn.Parameter(torch.zeros(self.inner_nodes))


        self.leaf_outputs = nn.Parameter(torch.randn(self.leaf_nodes, num_classes) * 0.1)

    def forward(self, x):
        batch_size = x.size(0)


        node_probs = torch.sigmoid(torch.matmul(x, self.node_weights.T) + self.node_bias)
        ...
        for d in range(self.depth):
            nodes_in_layer = 2 ** d
            split_probs = node_probs[:, :nodes_in_layer]
            node_probs = node_probs[:, nodes_in_layer:]


            left_probs = path_probs * (1 - split_probs)
            right_probs = path_probs * split_probs


            path_probs = torch.cat([left_probs, right_probs], dim=1)


        path_probs = path_probs.view(batch_size, self.leaf_nodes)


        outputs = torch.matmul(path_probs, self.leaf_outputs)
